...
| 项目编号 | 原文中的项目编号、采购项目编号、招标编号或项目编码，照抄原始编号（保留大小写、连字符和括号）；正文未写明时填`"null"`，不得用公告ID、包号或链接参数拼凑 |
...
项目编号、单位、地区、所属省/市、截止时间、预算、采购方式、科室和模型提供的医院全名如果没有字段证据，管线会保守改为`"null"`；医院索引自动补出的字段不需要模型重复举证。管线会把`所属省/市`统一归一化为`安徽`、`广西`、`北京`等简称，同时把`地区`统一为`安徽省凤阳县`、`广西壮族自治区南宁市`、`北京市朝阳区`等带省级全称的值。省份无法识别时，不保留孤立市县名。
...
运行内跨来源去重依次使用规范化`链接`、CCGP公告数字ID、PLAP公告ID、完整标题指纹、项目编号加公告阶段；官方来源优先于转载或聚合来源。跨运行与`seen.json`比较时，除链接、CCGP公告ID和PLAP公告ID外，还使用“完整标题指纹+发布时间+采购人指纹”，避免不同来源重复推送。
