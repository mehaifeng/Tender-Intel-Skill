...
#
# 检索引擎按分词匹配，短片段并不保证覆盖完整写法；Query 清单因此同时保留有边际召回的
# 片段和完整写法。筛选层必须识别这些写法及业务词表中的等价表达，否则检索捞回的公告会
# 在本地预筛被扔掉。宁可多留、由核实阶段按 references/verification.md 判掉。
...
#
# 末尾两组来自 2026-09-18 的销售反馈回测（飞书台账行 395–518，26 条判无效）：
# 科研级试剂盒与第三方质控品都不是临床可投标的标的。**两个词都必须收窄**——
# 裸的 `质控品` 会杀掉行 100、行 353 那两条判「有效」的标，它们是
# `过敏原特异性IgE抗体质控品`，正是本司产品线；裸的 `科研` 会杀掉行 61
# 昆明市儿童医院「科研相关采购项目…过敏原检测试剂」。按现在这个写法各命中
# 一条，且都是无效记录，无误杀。
EXCLUDE_TERMS = re.compile(
    r"酶标仪|电泳|兽医|兽用|畜牧|生猪|结核|干扰素释放|免疫组化|重组蛋白|培养基|缓冲液|核酸|PCR|测序"
    r"|科研试剂|科研用试剂|科研耗材|测试盒"
    r"|第三方质控品|室内质控品",
    re.I,
)
...
# 「值得打开正文看一眼」的门：标的物或标题里出现检验仪器、系统或试剂。
# 目标品类词全是试剂与抗体谱系名，仪器名本身不带信号，所以
# 「标的物只有一堆仪器、品类信号只在正文里」的公告会在列表层被整条丢掉——
# 2026-09-07 实测漏掉的军队采购意向（sm_names 全是仪器名，`变态反应` 只在正文里）
# 就是这样丢的。这道门只决定要不要花 1 积分取详情，留不留仍由正文的核心词说了算。
# 只认**检验类**仪器，不认泛泛的「仪」。2026-09-08 用真实 72h 窗口（439 条列表）实测：
# 放宽到任何「仪/仪器」会打开 56 条，把幼儿园教学仪器、智慧药房的分光光度计、
# 铁路保健站药品一起卷进来；收到检验类之后是 34 条，军队那条仍然保住。
LAB_ITEM_TERMS = re.compile(
    r"分析仪|检测仪|检验仪|免疫仪|发光仪|测定仪|筛查仪"
    r"|分析系统|检测系统|检验系统|筛查系统"
    r"|免疫[^\n、，,]{0,6}(?:流水线|工作站)|检验[^\n、，,]{0,6}(?:流水线|工作站)"
    r"|试剂|质控品|校准品|检测卡|诊断试纸|检测试纸",
    re.I,
)


def lab_item_term(*texts):
    """标的物或标题里的检验仪器/试剂类词；未命中返回空串。"""
    match = LAB_ITEM_TERMS.search("\n".join(t for t in texts if t))
    return match.group(0) if match else ""
...
def screen_domain(title_text, body_text=""):
    ...
    **另有最后一关：只命中 `CORROBORATION_ONLY_GROUPS` 的候选不入队**，因为那些组
    指向的品种公司根本没有产品可投；要么有核心组佐证，要么丢。这道关与上面几关
    不同，它不看排除词、只看信号落在哪几组，回测依据见 `CORROBORATION_ONLY_GROUPS`
    上方。它同样在列表阶段就生效，所以这类候选不会花掉取详情的积分。
    """
    title_text = title_text or ""
    ...
    # 只命中「无产品线」的宽片段组时不算数，要另有核心组佐证才入队。信号照原样带回：
    # 返空会让 zlbx_search 的 `weak = not keep and not signals` 误判成「没信号」，
    # 把这条候选又送进二次机会复核，白花一次取详情的积分。
    if all(name in CORROBORATION_ONLY_GROUPS for name in signals):
        return {
            "keep": False,
            "reason": "仅命中无本司产品线的宽片段组（{}），需核心组佐证".format(
                "、".join(signals)),
            "signals": signals,
...
BROAD_SIGNAL_GROUPS = frozenset({
    ...
    "25羟基维生素D",    # 业务方表里有，一线反馈「公司无相关产品」
    "细胞因子",        # 同上
})

# 只命中这些组时不算目标品类信号，必须另有核心组佐证才入队。
#
# 2026-09-18 用飞书台账行 395–518（124 条推送、26 条判无效）回测，并拿全表复核：
# `细胞因子` 独占的 22 条里 18 条被判无效（81.8%），是四组里唯一既够量又够差的。
# 业务方也已确认公司无对应产品线。
#
# **另外三组不能一起进来。** 同一次回测里 `印迹` 独占是 18 有效 / 8 无效，本区间内
# 宽片段独占的 4 条（行 442 军队医院、446 重庆妇幼、475 江西皮肤病、486 邯郸眼科）
# 全是真印迹仪采购机会；`25羟基维生素D` 与 `风湿(宽片段)` 样本太小，25羟基维生素D
# 独占全表也只有 2 条无效、0 条有效，再攒一轮数据再说。一刀切会误杀真标。
#
# 已知代价：会拦下 4 条判「有效」的细胞因子独占行（行 8、426、439、476），其中
# 426 与 476 的修改人是「多维表格助手」而非人工。这份名单已交业务方复核。
#
# 检索侧不动——18 组词表与 keywords.md 的 Query 清单原样保留，只是入队判定变了，
# 业务方若推翻结论，回滚这一处即可。
CORROBORATION_ONLY_GROUPS = frozenset({"细胞因子"})
...
_BODY_STUB_RULES = (
    ...
    (re.compile(r"详情请?见附件|详见附件|内容见附件|见公告附件|请下载附件|附件下载"), 400,
     "正文把主体内容指向附件；应另看 source_ext 解析文本或附件证据"),
