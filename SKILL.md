...
目标是快速得到可信的固定16字段情报。可插拔检索层默认联合睿销（jrbx）聚合库检索、中国政府采购网（CCGP）公开 HTTP 检索与军队采购网（PLAP）匿名公开检索；脚本负责跨来源去重、目标品类预筛、医院库匹配、字段校验和推送门禁，模型只核实当前小批次中的可信字段来源。
...
所有批次进入终态；所有推送载荷严格为固定16字段、单条、平铺、全字符串、无JSON null；成功回执已登记；摘要按来源披露检索失败，并披露跨来源重复、创建、排除、已有结论（`concluded`）、本地manual和推送成功数。

检索词与候选筛选的唯一依据是业务方《过敏》《自免》两张关键词表，落地在[关键词与Query](references/keywords.md)：**表里一行一条 query，项目代号一律进检索**。睿销与 CCGP 共用同一份 85 条清单（两者都是全文检索），PLAP 只公开标题检索、清单取核心名词层的 27 条。三个适配器都从 keywords.md 读清单，不各存副本；适配器参考文件只讲调用约束。

睿销是**精确子串匹配 + AND**，2026-09-03 实测**不支持 OR**（空格与竖线都只是分词符、分完仍取交集，`OR` 被当成第三个词，`+` 整串当一个词），query 合并不了；CCGP 抽测行为一致，两边因此共用一份清单。复测用 `python scripts/jrbx_search.py --probe-keyword-syntax`（7 次最小检索，不消耗回源配额）。
