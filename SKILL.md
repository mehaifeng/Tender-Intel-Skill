...
睿销的 Query 支持 AND 组词（`过敏原+试剂` 会传 `["过敏原","试剂"]`），不受旧豆包适配器「单个不含空格的词」限制。2026-09-03 起三个来源的清单已各自独立（睿销 23 条、CCGP 24 条、PLAP 14 条），keywords.md §5 只作历史实测记录，不再是任何适配器的运行清单。

产品域硬排除（`酶标仪`、`电泳`、兽用/科研/核酸等）定义在 `scripts/search_common.py` 的 `EXCLUDE_TERMS`，**三来源共用一份**，且在所有调用点都跑在品类信号之前。`化学发光` **不在**硬排除里：它 2026-09-03 已从三个信源的检索词与正向信号中移除，但硬排除会误伤「可与现有化学发光仪配套使用」这类本司试剂标（实测拦下过一条 54 万元的 ANCA 试剂盒招标），因此只摘信号不做排除——单说化学发光而无其它品类信号的公告由信号闸挡下，整机标由模型在核实阶段排除。依据见 [keywords.md §5.1](references/keywords.md)。
